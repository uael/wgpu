...
fn instance_err<E: std::error::Error + Send + Sync + 'static>(
...
impl Surface {
    pub(super) unsafe fn present(
        ...
        let sc = swapchain
            .as_ref()
            .ok_or_else(|| crate::SurfaceError::Other("Surface has no swap-chain configured"))?;
    ...
    unsafe fn configure(
                ...
                let native_window_ptr = match (self.wsi.kind, self.raw_window_handle) {
                    ...
                    (WindowKind::Wayland, Rwh::Wayland(handle)) => {
                        let library = &self
                            ...
                            .display_owner
                            .as_ref()
                            .ok_or_else(|| crate::SurfaceError::Other("No WSI display owner"))?
                            .library;
                        let wl_egl_window_create: libloading::Symbol<WlEglWindowCreateFun> =
                            unsafe { library.get(c"wl_egl_window_create".to_bytes()) }.map_err(
                                |err| {
                                    log::error!("Failed to load `wl_egl_window_create`: {err}");
        ...
        if let Some(window) = wl_window {
            ...
            let wl_egl_window_resize: libloading::Symbol<WlEglWindowResizeFun> =
                unsafe { library.get(c"wl_egl_window_resize".to_bytes()) }.map_err(|err| {
                    log::error!("Failed to load `wl_egl_window_resize`: {err}");
                    crate::SurfaceError::Other("Failed to load `wl_egl_window_resize")
                })?;
            unsafe {

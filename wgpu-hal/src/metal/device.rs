...
impl super::Device {
    fn load_shader(
        ...
        let function = library
            .new_function_with_name(&NSString::from_str(translated_ep_name))
            .ok_or_else(|| {
...
impl crate::Device for super::Device {
                ...
                for (entry, layout) in layout_and_entry_iter {
                    ...
                    if layout.count.is_some() {
                        ...
                        let buffer = self
                            ...
                            .device
                            .lock()
                            .new_buffer_with_length_options(
                                8 * count as usize,
    ...
    unsafe fn create_shader_module(
        ...
        match shader {
            ...
            crate::ShaderInput::Msl {
                ...
                let library = device
                    .new_library_with_source_options_error(
                        &NSString::from_str(&source),
                        Some(&options),
                    )
                    .map_err(|e| crate::ShaderError::Compilation(format!("MSL: {e:?}")))?;
                let function = library
                    .new_function_with_name(&NSString::from_str(&entry_point))
                    .ok_or_else(|| {
    ...
    unsafe fn create_compute_pipeline(
        ...
        autoreleasepool(|_| {
            ...
            let raw = unsafe {
                msg_send![&**self.shared.device.lock(), newComputePipelineStateWithDescriptor: &*descriptor, error: _]
            };
    ...
    unsafe fn create_fence(&self) -> DeviceResult<super::Fence> {
        ...
        let shared_event = if self.shared.private_caps.supports_shared_event {
            Some(self.shared.device.lock().new_shared_event().unwrap())
        } else {
            None
        };
        Ok(super::Fence {

...
use objc2_metal::{
    MTLBlitCommandEncoder, MTLBlitPassDescriptor, MTLCommandBuffer, MTLCommandEncoder,
    ...
    MTLRenderPassDescriptor, MTLScissorRect, MTLSize, MTLStoreAction, MTLTexture, MTLViewport,
...
impl crate::CommandEncoder for super::CommandEncoder {
            ...
            for (i, at) in desc.color_attachments.iter().enumerate() {
                if let Some(at) = at.as_ref() {
                    ...
                    if let Some(depth_slice) = at.depth_slice {
                        at_descriptor.set_depth_plane(depth_slice as usize);
                    }
    ...
    unsafe fn set_bind_group(
        ...
        if let Some(ref encoder) = self.state.render {
            ...
            for (resource, use_info) in group.resources_to_use.iter() {
                encoder.use_resource_usage_stages(
                    resource.as_ref(),
                    use_info.uses,
                    use_info.stages,
                );
        ...
        if let Some(ref encoder) = self.state.compute {
            ...
            for (resource, use_info) in group.resources_to_use.iter() {
                ...
                encoder.use_resource_usage(resource.as_ref(), use_info.uses);
            }
        }
    }
    ...
    unsafe fn dispatch(&mut self, count: [u32; 3]) {
        if count[0] > 0 && count[1] > 0 && count[2] > 0 {
            ...
            encoder
                .dispatch_threadgroups_threads_per_threadgroup(raw_count, self.state.raw_wg_size);
        }
    }
